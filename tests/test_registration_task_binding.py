...
from src.database.models import Base, Account, EmailService, RegistrationTask
...
def test_sync_registration_task_skips_registered_outlook_service_when_email_only_differs_by_case(monkeypatch):
    manager = _build_manager("registration_task_registered_outlook_normalized.db")

    with manager.session_scope() as session:
        first = EmailService(
            service_type="outlook",
            name="MiXeD@example.com",
            config={"email": "MiXeD@example.com", "password": "pass-1"},
            enabled=True,
            priority=0,
        )
        second = EmailService(
            ...
            config={"email": "fresh@example.com", "password": "pass-2"},
            enabled=True,
            priority=1,
        )
        session.add_all([first, second])
        ...
        session.add(
            RegistrationTask(
                task_uuid="task-normalized-skip",
                status="pending",
            )
        )

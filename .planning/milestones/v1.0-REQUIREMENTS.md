...
- [x] **COMP-01**: Existing clients can call every remaining Python-only backend capability through a Go-owned route using the same path and HTTP method.
- [x] **COMP-02**: Existing clients receive compatible JSON field names, status values, and error semantics from migrated Go endpoints.
...
- [x] **DATA-01**: Existing persisted records for accounts, settings, upload services, proxies, bind-card tasks, app logs, and team entities remain readable and writable after Go takeover without manual reshaping.
...
- [x] **CUT-02**: Production deployment can disable Python backend responsibilities with rollback instructions and parity verification evidence in place.
- [x] **OPS-01**: Go backend operational controls are sufficient to replace Python backend duties safely in production for all migrated domains.
